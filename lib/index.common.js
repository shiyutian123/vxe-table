...
/* harmony default export */ var footer = ({
  ...
  methods: {
    ...
    scrollEvent: function scrollEvent(evnt) {
      ...
      var isX = scrollLeft !== lastScrollLeft;
      $xetable.lastScrollLeft = scrollLeft;
      $xetable.lastScrollTime = Date.now(); // 防止页面抖动
      // if (headerElem) {
      ...
      if (!inpAttrs.readonly) {
        if (isNumber) {
          if (value) {
            ...
            this.emitUpdate(type === 'float' ? xe_utils_amd_xe_utils_default.a.toFixedString(inputValue, xe_utils_amd_xe_utils_default.a.toNumber(digits)) : '' + inputValue, {
              type: 'check'
            });
          }
